<!--
  CustomPersonalitiesSettingsSection.vue — Create, edit, and delete custom AI
  personalities owned by the current authenticated user.

  @section Quota
  Each user can own up to {@link MAX_CUSTOM_PERSONALITIES} custom personalities
  (currently 3). The form disables creation when the quota is exhausted and shows
  a warning alert. Existing personalities can still be edited and deleted.

  @section Prompt Format
  Prompt text is stored verbatim as markdown and injected as the system prompt
  at chat time. No sanitization or transformation is applied here.

  @section Deletion Side-Effects
  Deleting a personality that is currently selected as the active personality
  in the chat UI automatically resets the selection to "friendly".

  @composable useCustomPersonalities — handles CRUD and quota validation.
  @composable usePersonality — provides the reactive "personality" cookie ref.
-->
...
<template>
  <div class="space-y-6">
    <section class="rounded-2xl border border-default bg-elevated/40 p-4">
      ...
      <div class="grid gap-4">
        ...
        <UFormField
          ...
          required
          :error="
            promptTooLong ? `Maximum ${MAX_CUSTOM_PERSONALITY_PROMPT_LENGTH} characters` : undefined
          "
          class="w-full"
        >
          <UTextarea
    ...
    <section>
      ...
      <p class="mt-3 text-xs text-muted">{{ remainingSlots }} personality slots available.</p>
    </section>
  </div>
</template>
